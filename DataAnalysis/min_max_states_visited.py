...
def find_min_visited_states(games_data, fish_data, looking_glass_data, video_advanced_data):
    ...
    for lg_game in looking_glass_data:
        if isinstance(lg_game, dict):
            ...
            states = int(lg_game.get('states_checked', float('inf')))
    ...
    for va_game in video_advanced_data:
        if isinstance(va_game, dict):
            ...
            states = int(va_game.get('states_checked', float('inf')))
...
def find_max_visited_states(games_data, fish_data, looking_glass_data, video_advanced_data):
    ...
    for lg_game in looking_glass_data:
        if isinstance(lg_game, dict):
            game_won = lg_game.get('solvable', False)
            states = int(lg_game.get('states_checked', 0))
    ...
    for va_game in video_advanced_data:
        if isinstance(va_game, dict):
            game_won = va_game.get('solvable', False)
            states = int(va_game.get('states_checked', 0))
...
def main():
    ...
    video_advanced_directory = "/home/tautas/IdeaProjects/MasterT/Analysis/fc-solve-data/batch_solutions_video_editing"
    ...
    video_advanced_data = read_path_data(video_advanced_directory)

    min_states = find_min_visited_states(games_data, fish_data, looking_glass_data, video_advanced_data)
    max_states = find_max_visited_states(games_data, fish_data, looking_glass_data, video_advanced_data)

    print("Games with the minimum amount of visited states:")
    for algo, data in min_states.items():
        print(f"{algo}: Game number {data['gameNo']}, States visited {data['states']}")

    print("\nGames with the maximum amount of visited states:")
