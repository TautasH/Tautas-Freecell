...
def calculate_statistics(games_data):
    ...
    stats = {
        'Best Bucket First Search': {'counters': [], 'times': [], 'path_lengths': [], 'wins': 0, 'won_counters': [],
                                     'won_path_lengths': [], 'game_numbers': [], 'won_game_numbers': []},
        'MP Best Bucket First Search': {'counters': [], 'times': [], 'path_lengths': [], 'wins': 0, 'won_counters': [],
                                        'won_path_lengths': [], 'game_numbers': [], 'won_game_numbers': []}
    }
    ...
    for game in games_data:
        ...
        if algo_data_bbf:
            ...
            if path_bbf is not None:
                stats['Best Bucket First Search']['path_lengths'].append(
                    len(game['best_bucket_first_search'].get('path', [])))
            if game['best_bucket_first_search'].get('game_won'):
                ...
                stats['Best Bucket First Search']['won_path_lengths'].append(
                    len(game['best_bucket_first_search'].get('path', [])))
        ...
        if algo_data_mpbbf:
            stats['MP Best Bucket First Search']['counters'].append(
                game['mp_best_bucket_first_search'].get('counter', 0))
            stats['MP Best Bucket First Search']['game_numbers'].append(game_number)
            if path_mpbbf:
                stats['MP Best Bucket First Search']['path_lengths'].append(
                    len(game['mp_best_bucket_first_search'].get('path', [])))
            if game['mp_best_bucket_first_search'].get('game_won'):
                stats['MP Best Bucket First Search']['wins'] += 1
                stats['MP Best Bucket First Search']['won_counters'].append(
                    game['mp_best_bucket_first_search']['counter'])
                stats['MP Best Bucket First Search']['won_path_lengths'].append(
                    len(game['mp_best_bucket_first_search'].get('path', [])))
    ...
    print(
        f"{'Wins':<40} {stats['Best Bucket First Search']['wins']:<40} {stats['MP Best Bucket First Search']['wins']:<20}")
    print(
        f"{'Total states visited':<40} {sum(stats['Best Bucket First Search']['counters']):<40} {sum(stats['MP Best Bucket First Search']['counters']):<20}")
    print(
        f"{'States visited (won games)':<40} {sum(stats['Best Bucket First Search']['won_counters']):<40} {sum(stats['MP Best Bucket First Search']['won_counters']):<20}")

    for stat_name, stat_data in [('Average states visited', 'counters'),
                                 ('Average states visited (won games)', 'won_counters'),
                                 ('Average time taken (seconds)', 'times'),
                                 ('Average path length (won games)', 'won_path_lengths')]:
        row = [stat_name] + [
    ...
    for game in games_data:
        if (game.get('best_bucket_first_search') and game['best_bucket_first_search'].get('path') is None and not game[
            'best_bucket_first_search'].get('game_won')) or \
                (game.get('mp_best_bucket_first_search') and game['mp_best_bucket_first_search'].get(
                    'path') is None and not game['mp_best_bucket_first_search'].get('game_won')):
    ...
    print(
        f"{'Max path length (Best Bucket First Search)':<35} {max_path_length_bbf:<20} Game number: {max_path_game_bbf}")
    print(
        f"{'Max path length (MP Best Bucket First Search)':<35} {max_path_length_mpbbf:<20} Game number: {max_path_game_mpbbf}")
    if null_path_game_numbers:
        print(f"Games with no solution: {', '.join(map(str, null_path_game_numbers))}")


def find_missing_games(games_data, total_games=32000):
    present_game_numbers = set(game.get('game_number') for game in games_data if game.get('game_number') is not None)
    expected_game_numbers = set(range(1, total_games + 1))
    missing_game_numbers = expected_game_numbers - present_game_numbers
    return sorted(missing_game_numbers)
...
def main():
    ...
    missing_games = find_missing_games(games_data)
    print(f"\nNumber of missing games: {len(missing_games)}")
    if missing_games:
        print(f"Missing games: {', '.join(map(str, missing_games))}")
