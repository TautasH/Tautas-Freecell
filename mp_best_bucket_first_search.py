...
def initialize_buckets(num_buckets=10000):
...
def hFunction(state, move_count):
    # Simple heuristic: Count cards not in foundations and add 0.23 times the move count.
    return int((sum(13 - len(stack) for stack in state['foundations'].values()) + 0.23 * move_count) * 100)


def mp_best_bucket_first_search(cap=5000, initial_state=None):
    ...
    move_counts = {initial_hash: 0}  # Track the number of moves for each state
    current_state_hash = initial_hash

    initial_score = hFunction(initial_state, move_counts[initial_hash])
    if initial_score >= len(buckets):
        buckets.extend([] for _ in range(initial_score - len(buckets) + 1))
    ...
    while any(buckets) and counter < cap:
        for score, bucket in enumerate(buckets):
            if bucket:
                ...
                current_move_count = move_counts[current_state_hash]  # Get the move count for this state
                break
        else:
        ...
        for move in generate_possible_moves(current_state):
            ...
            if new_state_hash not in states_checked:
                ...
                move_counts[new_state_hash] = new_move_count
                new_score = hFunction(new_state, new_move_count)
                if new_score >= len(buckets):
                    buckets.extend([] for _ in range(new_score - len(buckets) + 1))
...
if __name__ == "__main__":
    ...
    for game_no in range(1, total_runs + 1):
        ...
        check, path, counter, total_time = mp_best_bucket_first_search(initial_state=init_state, cap=200000)
